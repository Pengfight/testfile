...
def load_city_data():
    ...
    with open('path_json_with_time1.json',encoding='utf-8') as fd:
        loaded = json.load(fd)
    outfile1=open('cityHours_doc_train_dis_4.txt','w',encoding='utf-8')
    outfile2=open('cityHours_doc_train_dis_4-5.txt','w',encoding='utf-8')
    outfile3=open('cityHours_doc_train_dis_6-7.txt','w',encoding='utf-8')
    outfile4=open('cityHours_doc_train_dis_8-10.txt','w',encoding='utf-8')
    outfile5=open('cityHours_doc_train_dis_10.txt','w',encoding='utf-8')
    for path in loaded:
        '''rid+=1
        tid+=1
        ...
        if len(citys_arry)<4:
            rid1+=1
            outfile1.write(str(rid1)+' '+str(rid1)+' ')
            outfile1.write(citylist)
            outfile1.write('\n')
        elif len(citys_arry)<=5:
            rid2+=1
            outfile2.write(str(rid2)+' '+str(rid2)+' ')
            outfile2.write(citylist)
            outfile2.write('\n')
        elif len(citys_arry)<=7:
            rid3+=1
            outfile3.write(str(rid3)+' '+str(rid3)+' ')
            outfile3.write(citylist)
            outfile3.write('\n')
        elif len(citys_arry)<=10:
            rid4+=1
            outfile4.write(str(rid4)+' '+str(rid4)+' ')
            outfile4.write(citylist)
            outfile4.write('\n')
        else:
            rid5+=1
            outfile5.write(str(rid5)+' '+str(rid5)+' ')
